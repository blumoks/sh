...
*/
UserFunctionality::UserFunctionality():KeyManagmentClass(){};
/**
...
*/
UserFunctionality::UserFunctionality(char *pcDeviceID, MY_PIN_PARAMS sPinParams):KeyManagmentClass(pcDeviceID,sPinParams){};
/**
...
#pragma endregion
	...
	GetKeysInfoList(sMyTemplate,psMyKeyList,ulKeyNumber);
	if (CKR_OK != rvResult)
	{
		if (sMyTemplate.psKeyParams) free (sMyTemplate.psKeyParams);
		return;
...
#pragma endregion
	...
	GenerateSecKey(sMyTemplate,NULL);
	if (sMyTemplate.psKeyParams) free (sMyTemplate.psKeyParams);
...
#pragma endregion
	GenerateKeyPair(sMyPriTemplate,sMyPubTemplate,NULL);
	if (sMyPriTemplate.psKeyParams) free (sMyPriTemplate.psKeyParams);
	if (sMyPubTemplate.psKeyParams) free (sMyPubTemplate.psKeyParams);
	if (pcCkaIdValue) free (pcCkaIdValue);
};
...
#pragma endregion
	...
	ExportPublicKey(sMyTemplate,pbExportedKey,ulExportedKeyLen);
	if (sMyTemplate.psKeyParams) free (sMyTemplate.psKeyParams);
	if (pcCkaIdValue) free (pcCkaIdValue);
};
//------------------------------------------------------------------------------------------------------------------
...
#pragma region MakeExportPriSecKey
	ExportSecPriKey(sSecPriKeyTemplate,sPubKeyTemplate,sPriKeyTemplate,pbExportedKey,ulExportedKeyLen);
#pragma endregion
...
#pragma region MakeExportPriSecKey
	ImportSecPriKey(sPubKeyTemplate,sPriKeyTemplate,pbExportedKey,ulExportedKeyLen);
#pragma endregion
EXPORT_SECPRIKEY_BY_STRINGS:
#pragma region ExportSecPriKeyByStringsFinalization
	pcPubKeyInfo = NULL;
...
void UserFunctionality::ImportPublicFromFile(char **pcInputData, CK_ULONG ulNumOfParams)
{
	...
	if ((rvResult == CKR_OK)&&(ulTempBufferLen))
	{
		...
		if (rvResult == CKR_OK)
		{
			ImportPublicKey(pbTempBuffer,ulTempBufferLen);
		}
	}
	if (pbTempBuffer) free (pbTempBuffer);
};
